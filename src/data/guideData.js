...
export const GRIND_GUIDE = [
  {
    size: "Extra Coarse",
    color: "#c8a878",
    desc: "Visibly chunky, like cracked peppercorns or coarse sea salt. Almost no resistance when you rub it between your fingers. Used exclusively for cold brew - the 12-24 hour steep compensates for the open grind, and going finer would make the concentrate bitter and astringent.",
    methods: ["Cold Brew"],
  },
  {
    size: "Coarse",
    color: "#b89060",
    desc: "Like rough kosher salt - distinct, irregular particles with plenty of space between them. Ideal for French Press because the full-immersion brew method needs a coarse grind to avoid over-extraction during the 4-minute steep. Also works well for cold brew concentrate.",
    methods: ["French Press", "Cold Brew"],
  },
  {
    size: "Medium-Coarse",
    color: "#c09858",
    desc: "Slightly finer than French Press but still quite open - closer to coarse beach sand. The Chemex uses a much thicker paper filter than a V60, which dramatically slows the flow rate. A coarser grind compensates for this, keeping brew time in range and preventing a bitter, over-extracted cup.",
    methods: ["Chemex"],
  },
  {
    size: "Medium",
    color: "#b88848",
    desc: "The workhorse grind - consistent, uniform particles about the size of fine breadcrumbs. Drip machines are designed around this grind size, and it's the safest starting point for any new brew method. Not too fast, not too slow, extracts evenly with moderate heat and contact time.",
    methods: ["Drip Machine"],
  },
  {
    size: "Medium-Fine",
    color: "#c09040",
    desc: "Finer than drip but not as dense as espresso - smooth to the touch with just a hint of texture. This is the sweet spot for pour over and AeroPress, where you want enough resistance to slow the flow and extend contact time, but not so much that it chokes the drawdown or over-extracts.",
    methods: ["Pour Over / V60", "AeroPress"],
  },
  {
    size: "Fine",
    color: "#a87838",
    desc: "Dense and powdery-smooth, like fine table salt or granulated sugar. At this size, water has to work hard to push through the bed of grounds, which is exactly what espresso machines and Moka Pots need. That resistance is what creates pressure and forces fast, concentrated extraction in under 30 seconds.",
    methods: ["Espresso", "Moka Pot"],
  },
  {
    size: "Extra Fine",
    color: "#906030",
    desc: "Almost a powder - finer than espresso, closer to flour or powdered sugar. The only common use is Turkish coffee, where grounds are simmered directly in water in a cezve and never filtered out. The ultra-fine grind is intentional: it settles to the bottom of the cup and becomes part of the drinking experience.",
  ...
  return (
    <div className="guide-grind-section">
      ...
      <p className="guide-grind-intro">
        Click a roast level to explore its characteristics, flavor tendencies, and brewing tips.
      </p>
      ...
      <div className="roast-track">
        {ROAST_GUIDE.map((r) => (
          ...
          >
            <div className="roast-bean-icon" style={{ color: tc(r.color) }}>
              {r.icon}
            </div>
      ...
      {active && (
        <div className="roast-detail" style={{ borderTopColor: tc(active.color) }}>
          <div className="roast-detail-top">
            <div>
              <div className="roast-detail-level" style={{ color: tc(active.color) }}>
                {active.level} Roast
              </div>
          ...
          <div className="roast-chars">
            {active.characteristics.map((c) => (
              <div key={c} className="roast-char">
                <span className="roast-char-dot" style={{ background: tc(active.color) }} />
                {c}
              </div>
            ))}
          </div>
          <div className="roast-bestfor">
            <span className="roast-bestfor-label">Best for: </span>
            {active.bestFor}
          </div>
...
export function MilkGuide({ tc }) {
  ...
  return (
    <div className="guide-grind-section">
      ...
      <p className="guide-grind-intro">
        Click any milk to see its hot and iced flavor profile, how well it steams, and which drinks
        it works best in.
      </p>
      ...
      <div className="milk-guide-track">
        {MILK_GUIDE.map((m) => (
          ...
          >
            <span className="milk-guide-icon" style={{ color: tc(m.color) }}>
              {m.icon}
            </span>
      ...
      {active && (
        <div className="milk-guide-detail" style={{ borderTopColor: tc(active.color) }}>
          <div className="milk-guide-detail-header">
            <div className="milk-guide-detail-name" style={{ color: tc(active.color) }}>
              {active.name}
            </div>
...
export const ORIGINS_GUIDE = [
  ...
  },
  {
    ...
    roastRec:
      "Medium to Dark. Brazilian beans shine at medium-dark where the chocolate and nut notes develop fully.",
