...
// --- Bean Journal -------------------------------------------------------------
function BeanJournal({ onBrewCalc, onBeansChange, addTrigger, showToast, session, onViewChange, shareTrigger, onNeedAuth }) {
            ...
            <div style={{ border: "1px solid var(--border)", padding: 24 }}>
              ...
              {loadingFriends ? (
                ...
                friends.map(f => (
                  <div key={f.friendship_id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                    <span style={{ fontSize: 14, color: "var(--text)" }}>@{f.screenname}</span>
                    <button className="btn-ghost" style={{ fontSize: 11, padding: "4px 10px", color: "var(--red)" }} onClick={() => handleRemoveFriend(f.friendship_id)}>Remove</button>
                  </div>
                ))
              )}
            </div>
...
function ProfileFeedItem({ item, formatDate, REACTIONS, reactionCount, myReactions, handleReact, session, profile }) {
  const [expanded, setExpanded] = useState(false);
  return (
    ...
    <div className="page">
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        ...
        {loading ? (
          ...
          feed.map(item => (
            <ProfileFeedItem key={item.id} item={item} formatDate={formatDate} REACTIONS={REACTIONS} reactionCount={reactionCount} myReactions={myReactions} handleReact={handleReact} session={session} profile={profile} />
          ))
        )}
      </div>
    </div>
